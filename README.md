...
564=将路径点上的指定科技类型关联到指定标签... (PhobosExt),-4,46,38,30,15,0,0,0,0,将指定路径点上的指定科技类型关联到指定指定标签。布尔值用来指定是否优先创建一个新的标签实例(TagClass)。(若科技类型为建筑，则每一格都会检测，不支持ares的自定义Foundation，详细细节请见readme.md里的相关介绍),0,1,564,1
565=将路径点上指定所属方的指定科技类型关联到指定标签... (PhobosExt),-4,46,38,30,2,15,0,0,0,将指定路径点上指定所属方的指定科技类型关联到指定指定标签。布尔值用来指定是否优先创建一个新的标签实例(TagClass)。(若科技类型为建筑，则每一格都会检测，不支持ares的自定义Foundation，详细细节请见readme.md里的相关介绍),0,1,565,1
