...
> - **范围**：整数，以单元格为单位的圆形半径 。
> - **布尔值**：`0` 或 `1`，控制是否强制创建一个新的标签实例。（见下文注意事项第8条）

#### 重要注意事项

1. 使用本触发时，会更新指定标签的 `InstanceCount`（引用计数）。  
2. 由本触发创建的标签可以被原版动作 `70`（摧毁关联对象）安全销毁而不会弹框。  
3. 即使想要绑定的科技类型**正在建造中**，也会被关联到指定标签。  
4. 通过此触发给小队成员关联的标签，在小队解散后标签不会自动消失。  
5. 对于动作 `564`、`565`，若科技类型为建筑，则**每一格都会检测**，当前**不支持 Ares 自定义建筑地基**。  
6. 当指定对象已经拥有标签时，新标签会**覆盖**原有标签，并且原有标签的 `InstanceCount` 会相应更新。  
7. 与常规销毁（如动作 `70`）不同，动作 `563` **不会导致由小队创建的标签被销毁时弹框崩溃**，因此可安全用于动态标签的批量清理。  
8. **布尔值参数（强制新建）**：  
